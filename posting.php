	...
	if (!sizeof($error) && $submit)
	{
		...
		if ($submit)
		{
			...
			if ($Points['points_enable']) {
				switch ($mode) {
					case "post":
						$Points = ($Points['points_pertopic']) ? $post_data['forum_pertopic'] : 0;
					break;
					case "reply":
						$Points = ($Points['points_perpost']) ? $post_data['forum_perpost'] : 0;
					break;
					case "edit":
						$Points = ($Points['points_peredit']) ? $post_data['forum_peredit'] : 0;
					break;
					case "quote":
						$Points = ($Points['points_perpost']) ? $post_data['forum_perpost'] : 0;
					break;
			...
			$data = array(
				...
				//--[Points Modification]--
				'points'				=> $Points,
				'points_post'           => (int) $post_data['points_post'],
				//--[Points Modification]--
			...
			else
			{
				...
				//--[Points Modification]--
// Display the amount of points win.
			if ($auth->acl_get('u_use_points') AND $Points['points_enable'])
            {
			         switch ($mode) {
			           case "reply":
			           case "post":
			           case "quote":
				             ...
				             $sql = 'SELECT points_post
						             FROM ' . POSTS_TABLE . '
						             WHERE post_id = ' . $data['post_id'];
				             $result = $db->sql_query($sql);
			                 $tb = $db->sql_fetchrow($result);
			                 $db->sql_freeresult($result);

					$points_redirecmsg = sprintf($user->lang['POINTS_REDIRECT_MSG'], $tb['points_post'], $Points['points_name']);
			        }
		    }
				//--[Points Modification]--

				$message = ($mode == 'edit') ? 'POST_EDITED' : 'POST_STORED';
$message = $user->lang[$message] .'<br /> ' . $points_redirecmsg . '<br />' . sprintf($user->lang['VIEW_MESSAGE'], '<a href="' . $redirect_url . '">', '</a>');
			}
	...
	if ($auth->acl_get('m_delete', $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get('f_delete', $forum_id) && $post_id == $post_data['topic_last_post_id'] && !$post_data['post_edit_locked'] && ($post_data['post_time'] > time() - ($config['delete_time'] * 60) || !$config['delete_time'])))
	{
		...
		if (confirm_box(true))
		{
			$data = array(
				...
				//--[Points Modification]--
				'points_post'           => $post_data['points_post'],
				//--[Points Modification]--
